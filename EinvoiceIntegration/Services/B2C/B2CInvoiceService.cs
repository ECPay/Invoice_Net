...
namespace EinvoiceIntegration.Services.B2C
{
    public class B2CInvoiceService
    {
        public B2CInvoiceService()
        {
        ...
        private B2CInvoiceMethod _B2C;
        public B2CInvoiceMethod B2CEnum
        {
            get { return _B2C; }
            set { _B2C = value; }
        }
            ...
            where S : BaseResult, new()
        {
            ...
            if (IsCheckModel)
            {
                ...
                if (!IsValid)
                {
                    var data = new BaseResult { RtnCode = 0, RtnMsg = errorMsg };
            ...
            where S : BaseResult
        {
            ...
            var urlModel = IApi.GetList().Where(t => t.Category == EinvoiceCategory.B2C && t.env == EnvEnum).FirstOrDefault();

            string apiUrl = string.Format("{0}/{1}", urlModel.apiUrl, B2CEnum.ToString());
